...
public class Settings {
    ...
    private String getAlsaSettings() {
        ...
        settings.append("pcm.dshare {\n" + "  type dmix\n" + "  ipc_key 2048\n" + "  slave {\n" + "    pcm \"hw:").append(audioDevice.getKey()).append("\"\n").append("    rate ").append(audioRate).append("\n").append("    channels ").append(getTotalAudioChannels()).append("\n").append("  }\n").append("  bindings {\n");

        // Add all channels
        for (int i = 0; i < getTotalAudioChannels(); i++) {
            settings.append("    ").append(i).append(" ").append(i).append("\n");
        }
        ...
        for (int i = 0; i < audioBusList.size(); i++) {
            ...
            for (int j = 0; j < audioBus.getChannels(); j++) {
                settings.append("  ttable.").append(j).append(".").append(currentChannel).append(" 1\n");
